...
Additionally, create a `wrangler.toml` file at the root of your project. This file will serve as the configuration for our Worker. To kick off the configuration, add the following to the file, replacing the worker name and entry point with your corresponding values:
...
Finally, on each button click we call the following function to update all the values:
