...
I have recently added the possibility to like the posts of this blog. To implement this feature, I needed to create an api endpoint and persist the likes data in a database.

In situations like these, where I need to implement some modest backend functionality in a static site, my go-to technology is Cloudflare. With a very generous free tier, Cloudflare comes with this incredible thing called Workers which allows you to quickly set up serverless api endpoints, instantly deployed across their Edge network spanning 300 global locations. To top it, Cloudflare has introduced a database solution, D1 (in open alpha by the time this was written).

Since I wanted to take full advantage of the Workers Edge runtime, I used these technologies that support it:
...
In this blog post, I'll guide you through the process I followed to add blog by adding a post liking functionality using these technologies. This approach can be extended to incorporate various backend features, making it helpful for anyone who wants to add these to their static sites.
