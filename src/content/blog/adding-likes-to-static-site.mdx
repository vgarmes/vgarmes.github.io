...
## Creating the first endpoint: Fetching post likes

Our initial objective is to defining an endpoint for retrieving both post and user likes. This endpoint will be called from the client every time a user accesses a blog post.

I've designated the route for this endpoint as `/api/posts/stats/:slug`, making the post slug easily accessible via the request parameters within the request context: `c.req.param('slug')`.

To retrieve the required data from our database, we'll utilize the database binding previously defined, which is also readily accessible within the request context.
...
Then we can consume the request context and initiate a new database connection as below:
...
Now, we just need to query the database to retreive the relevant information. My approach for this was to first fetch the statistics about the corresponding post using its slug and right after fetch the amount of likes associated with the specific user.

Putting everything together, our endpoint can return the post statistics as below:
...
## Storing user's likes

To store the user's likes, I just created a new endpoint similary to the one above but with the `POST` method. With Hono, this is as simple as creating the endpoint like below:
...
Without getting into many details, what this endpoint does is first identify the user and fetch the amount of likes this user has for the corresponding blog post. If this exceeds the set limits, we return an error. Otherwise we proceed to update both the posts and user likes database tables. You can check the source code as reference.
