---
title: Building Custom Code Blocks in Astro with MDX
description: Interactive code blocks with Shiki in Astro MDX
pubDate: 2025-11-06
...
Astro ships with excellent out-of-the-box support for Markdown, making it ideal for static content-heavy sites. However, styling Markdown has its limitations, especially when you want to customize certain elements like code blocks.

This is where Astro's [MDX integration](https://docs.astro.build/en/guides/integrations-guide/mdx/#why-mdx) comes in.
...
This `CodeBlock` can be any component you choose, but we with a few considerations:
...
The final touch is a copy button. For that, we can grab the source coude which is available in Shiki's transformers and pass it as a `data-code` attribute to our component:
...
For brevity, I ommited some of the logic like the hook to copy to clipboard. You can see the full implementation in the [source code](https://github.com/vgarmes/vgarmes.github.io/blob/main/src/components/copy-button.tsx).
