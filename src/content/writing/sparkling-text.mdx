...
The position is randomized between -20% and 120% of the parent element's width and height so the sparkles can also render slightly outside this element.
...
Finally we need to trigger the animation when the user is hovering on the text. For that we can add pointer event listeners:
...
Another thing to consider is that hovering is not possible on mobile devices, so for these we will always show the animation.

For that we can add a hook that checkes wheter hovering is supported and, if that's the case, trigger the animation automatically on mount:
...
You can see all the implementation details in the [source code](http://github.com/vgarmes/vgarmes.github.io/blob/main/src/components/demos/sparkling-text.tsx)
